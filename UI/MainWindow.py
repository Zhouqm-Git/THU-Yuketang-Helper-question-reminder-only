...
"font: 9pt \"微软雅黑\";")
        ...
        self.add_message_signal.emit("雨课堂答题提醒助手 v1.0",0)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "雨课堂答题提醒助手"))
        self.label_2.setText(_translate("MainWindow", "雨课堂答题提醒"))
    ...
    def check_config(self, config_route):
        """检查配置文件"""
        # 检查配置文件存在性及可用性
    ...
    def show_notification(self, message):
        ...
        try:
            ...
            threading.Thread(target=show_popup_notification, args=(message, 3), daemon=True).start()
    ...
    def active(self):
        ...
        self.add_message_signal.emit("答题提醒功能已启动", 0)
        
        # 获取课程列表并开始监听
        self.start_lesson_monitoring()
    
    def start_lesson_monitoring(self):
        """开始课程监听"""
        try:
            # 获取正在进行的课程
            code, lessons = get_on_lesson(self.config["sessionid"])
            if code == 0 and lessons:
                self.add_message_signal.emit(f"检测到 {len(lessons)} 门正在进行的课程", 0)
                ...
                for i, lesson_data in enumerate(lessons):
                    # 添加到课程列表
                    row = [
                        lesson_data.get("courseName", "未知课程"),
                        lesson_data.get("lessonName", "未知课程标题"),
                        lesson_data.get("teacherName", "未知教师"),
                        lesson_data.get("startTime", "未知时间")
                    ]
                    self.add_course_signal.emit(row, i)
                    
                    # 创建课程监听线程
                    lesson_thread = threading.Thread(
                        target=self.monitor_lesson,
                        args=(lesson_data,),
                        daemon=True
                    )
                    lesson_thread.start()
                    ...
                    self.add_message_signal.emit(f"开始监听课程: {lesson_data.get('courseName', '未知课程')}", 0)
            else:
                self.add_message_signal.emit("当前没有正在进行的课程", 0)
        except Exception as e:
            self.add_message_signal.emit(f"获取课程列表失败: {str(e)}", 0)
    
    def monitor_lesson(self, lesson_data):
        """监听单个课程"""
        try:
            ...
            self.add_message_signal.emit(f"课程 {lesson_data.get('courseName', '未知')} 数据解析错误: {str(e)}", 0)
        except Exception as e:
            self.add_message_signal.emit(f"课程 {lesson_data.get('courseName', '未知')} 监听错误: {str(e)}", 0)
    ...
    def deactive(self):
        ...
        self.is_active = False
        self.lesson_threads.clear()  # 清空线程列表
        
        self.active_btn.setEnabled(True)
        self.active_btn.setText("启动")
        self.add_message_signal.emit("答题提醒功能已停止", 0)